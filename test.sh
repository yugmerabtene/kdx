...
cleanup_artifacts() {
  rm -f ci_out.s ci_out.o ci_out_bin ci_hello.s ci_hello_bin ci_flow.s ci_big.kdx /tmp/kdx_test.stderr
}
...
echo "[1/10] Help output"
./kdx --help >/dev/null

echo "[2/10] Assembly-only mode"
./kdx examples/simple.kdx -S -o ci_out.s
test -f ci_out.s

echo "[3/10] Compile-only mode"
./kdx examples/simple.kdx -c -o ci_out.o
test -f ci_out.o

echo "[4/10] Full compile and run"
./kdx examples/simple.kdx -o ci_out_bin
test -f ci_out_bin
chmod +x ci_out_bin
./ci_out_bin

echo "[5/10] Hello sample (typed return + call syntax)"
...
echo "[6/10] Control-flow sample emits assembly"
./kdx examples/control_flow.kdx -S -o ci_flow.s
test -f ci_flow.s

echo "[7/10] Invalid flag returns error"
expect_failure 1 ./kdx --bad-flag

echo "[8/10] Missing input returns error"
expect_failure 1 ./kdx

echo "[9/10] Missing file returns I/O error"
expect_failure 5 ./kdx examples/does_not_exist.kdx

echo "[10/10] Oversized input is rejected"
