...
def lane_rank(lane: str, focus_lanes: List[str], deprioritized_lanes: List[str]) -> int:
    if lane in focus_lanes:
        return focus_lanes.index(lane)
    if lane in deprioritized_lanes:
        return len(focus_lanes) + 100
    return len(focus_lanes) + 10


def task_sort_key(task: Dict, focus_lanes: List[str], deprioritized_lanes: List[str]) -> Tuple[int, int, str]:
    lane = str(task.get("lane", ""))
    return (
        priority_score(task.get("priority", "low")),
        lane_rank(lane, focus_lanes, deprioritized_lanes),
        str(task.get("id", "")),
    )
...
def regenerate_backlog(backlog: Dict) -> Dict:
    ...
    backlog["tasks"] = [
        ...
        {
            "id": f"SPRINT-REGGUARD-{generated}",
            "title": "Regression guard release safety gate",
            "lane": "regression_guard",
            "priority": "high",
            "status": "pending",
            "retries": 0,
            "commit_message": "chore(reg-guard): periodic regression safety gate",
            "steps": ["python3 ./.autodev/lanes/regression_guard.py"],
        },
        {
            "id": f"SPRINT-MINIMIZER-{generated}",
            "title": "Failure minimizer repro reducer",
            "lane": "minimizer",
            "priority": "medium",
            "status": "pending",
            "retries": 0,
            "commit_message": "chore(minimizer): periodic failure minimization",
            "steps": ["python3 ./.autodev/lanes/minimizer.py"],
        },
        {
            "id": f"SPRINT-RELEASE-{generated}",
            "title": "Release manager readiness snapshot",
            "lane": "release_manager",
            "priority": "medium",
            "status": "pending",
            "retries": 0,
            "commit_message": "chore(release): periodic readiness snapshot",
            "steps": ["python3 ./.autodev/lanes/release_manager.py"],
        },
        {
            "id": f"SPRINT-FLAKY-{generated}",
            "title": "Flaky hunter stability sweep",
            "lane": "flaky_hunter",
            "priority": "medium",
            "status": "pending",
            "retries": 0,
            "commit_message": "chore(flaky): periodic stability sweep",
        ...
        {
            "id": f"SPRINT-SECFAST-{generated}",
            "title": "Fast secret and safety scan",
            "lane": "security_fast",
            "priority": "high",
            "status": "pending",
            "retries": 0,
            "commit_message": "chore(security): fast secret and safety scan",
            "steps": [
        ...
        {
            "id": f"SPRINT-RELEASEOPS-{generated}",
            "title": "Release operations readiness",
            "lane": "release_ops",
            "priority": "high",
            "status": "pending",
            "retries": 0,
            "commit_message": "chore(release): release operations readiness pass",
            "steps": [
        ...
        {
            "id": f"SPRINT-WEBDOCS-{generated}",
            "title": "Website and docs integrity pass",
            "lane": "webdocs_ops",
            "priority": "medium",
            "status": "pending",
            "retries": 0,
            "commit_message": "docs(site): webdocs integrity and release status pass",
            "steps": [
                "python3 ./.autodev/lanes/web_launch_site.py",
                "python3 ./.autodev/lanes/webdocs_ops.py",
            ],
        },
        {
            "id": f"SPRINT-JURISTE-{generated}",
            "title": "Legal licensing policy audit",
            "lane": "juriste_license",
            "priority": "medium",
            "status": "pending",
            "retries": 0,
            "commit_message": "docs(legal): periodic licensing policy audit",
            "steps": ["python3 ./.autodev/lanes/juriste_license.py"],
        },
        {
...
def next_task(backlog: Dict, policy: Optional[Dict] = None) -> Optional[Dict]:
    pending = [t for t in backlog.get("tasks", []) if t.get("status") == "pending"]
    if not pending:
        return None

    policy = policy or {}
    focus_lanes = [str(x) for x in policy.get("focus_lanes", []) if isinstance(x, str)]
    deprioritized_lanes = [str(x) for x in policy.get("deprioritized_lanes", []) if isinstance(x, str)]

    if bool(policy.get("p0p1_only", False)):
        high_only = [t for t in pending if priority_score(str(t.get("priority", "low"))) == 0]
        if high_only:
            pending = high_only

    if bool(policy.get("strict_focus_only", False)) and focus_lanes:
        focused = [t for t in pending if str(t.get("lane", "")) in focus_lanes]
        if focused:
            pending = focused

    ordered = sorted(pending, key=lambda t: task_sort_key(t, focus_lanes, deprioritized_lanes))
    return ordered[0]
...
def current_branch() -> Optional[str]:
    cp = run_cmd("git rev-parse --abbrev-ref HEAD", check=False)
    branch = cp.stdout.strip()
    if cp.returncode != 0 or not branch or branch == "HEAD":
        return None
    return branch


def default_remote_branch() -> Optional[str]:
    cp = run_cmd("git symbolic-ref --quiet --short refs/remotes/origin/HEAD", check=False)
    value = cp.stdout.strip()
    if cp.returncode != 0 or not value:
        return None
    if "/" not in value:
        return None
...
def commit_and_tag(task: Dict, policy: Dict, state: Dict) -> None:
    ...
    if expected_branch.lower() == "auto" or not expected_branch:
        detected_branch = default_remote_branch()
        if detected_branch:
    ...
    if expected_branch and branch and branch != expected_branch:
        log(
            "Autocommit skipped: current branch "
            f"'{branch}' does not match expected '{expected_branch}'"
        )
        return
...
def main_loop(once: bool = False) -> int:
    ...
    if bool(policy.get("throughput_mode", False)):
        log(
            "Throughput mode enabled: "
            f"sleep={int(policy.get('loop_sleep_seconds', 60))}s "
            f"qa_loops={int(policy.get('qa_loops', 1))} "
            f"feature_every={int(policy.get('feature_every_n_validation_tasks', 3))}"
        )
    if bool(policy.get("p0p1_only", False)):
        log("Turbo filter enabled: high-priority lanes only")
    if bool(policy.get("strict_focus_only", False)):
        log("Turbo filter enabled: strict focus lane selection")
    ...
    while True:
        ...
        if feature_enabled:
            ...
            if feature_backlog.get("tasks"):
                feature_backlog = {"generated_at": now_utc(), "tasks": []}
                save_json(FEATURE_BACKLOG_PATH, feature_backlog)
                log("Feature backlog cleared because feature_enabled=false")

        validation_task = next_task(backlog, policy)
        feature_task = next_task(feature_backlog, policy) if feature_enabled else None
